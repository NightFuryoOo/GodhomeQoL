...
public sealed class MarmuP2Helper : Module
{
    ...
    private static bool ShouldUseCustomHp() => marmuP2UseMaxHp;
    ...
    private static void ApplyMarmuHealth(GameObject marmu, HealthManager? hm = null)
    {
        if (!ShouldApplySettings(marmu) || !ShouldUseCustomHp())
        {
            return;
        }

        hm ??= marmu.GetComponent<HealthManager>();
        if (hm == null)
        {
            return;
        }
        ...
        int targetHp = ClampMarmuHp(marmuP2MaxHp);
        marmu.manageHealth(targetHp);
        hm.hp = targetHp;
        TrySetMaxHp(hm, targetHp);
    }

    private static void RestoreVanillaHealth(GameObject marmu, HealthManager? hm = null)
    {
        if (marmu == null || !IsMarmuObject(marmu))
        {
            return;
        }

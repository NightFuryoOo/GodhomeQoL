...
public sealed class CollectorPhases : Module
{
    ...
    [LocalSetting]
    internal static bool ToL = false; // Tower of Love

    [LocalSetting]
    internal static bool HoG = false; // Hall of Gods attuned

    [LocalSetting]
    internal static bool HoG2 = false; // Hall of Gods ascended / radiant

    [LocalSetting]
    internal static bool Pantheons = false; // Pantheon of Hallownest
    ...
    private static void HandlePhaseControl(PlayMakerFSM fsm)
    {
        ...
        // Tower of Love
        if (scene.StartsWith("Ruins2_11", StringComparison.Ordinal))
        {
            fsm.ChangeFsmTransition("Init", "FINISHED", ToL ? "Phase 2" : "Set");
            return;
        }
    ...
    internal static IEnumerable<Element> MenuElements()
    {
        return new Element[]
        {
            ...
            Blueprints.HorizontalBoolOption(
                "Settings/CollectorPhases/TowerOfLove".Localize(),
                "",
                b => ToL = collectorPhase != 1 && b,
                () => ToL
            ),
            Blueprints.HorizontalBoolOption(
                "Settings/CollectorPhases/HoGAttuned".Localize(),
                "",
                b => HoG = collectorPhase != 1 && b,
                () => HoG
            ),
            Blueprints.HorizontalBoolOption(
                "Settings/CollectorPhases/HoGAscended".Localize(),
                "",
                b => HoG2 = collectorPhase != 1 && b,
                () => HoG2
            ),
            Blueprints.HorizontalBoolOption(
                "Settings/CollectorPhases/Pantheons".Localize(),
                "",
                b => Pantheons = collectorPhase != 1 && b,
                () => Pantheons
            ),
            Blueprints.HorizontalBoolOption(

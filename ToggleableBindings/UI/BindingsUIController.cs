...
namespace ToggleableBindings.UI
{
    ...
    {
        [System.Diagnostics.CodeAnalysis.NotNull] public static BindingsUIController? Instance { get; private set; }
        ...
        private bool CanOpenMenu()
        {
            ...
            if (hci.controlReqlinquished)
                return false;

            if (!GodhomeQoL.GodhomeQoL.GlobalSettings.BindingsMenuEverywhere && !IsAllowedBindingsScene())
                return false;

            if (Keybinds.OpenBindingsUI.IsPressed)
                return true;

            KeyCode hotkey = GetBindingsMenuHotkey();
            return hotkey != KeyCode.None && UnityEngine.Input.GetKeyDown(hotkey);
        }

        private static bool IsAllowedBindingsScene()
        {
            string sceneName = USceneManager.GetActiveScene().name;
            return sceneName == "GG_Workshop"
                || sceneName == "GG_Atrium"
                || sceneName == "GG_Atrium_Roof";
        }

        private static KeyCode GetBindingsMenuHotkey()
        {
            string? raw = GodhomeQoL.GodhomeQoL.GlobalSettings.BindingsMenuHotkey;
